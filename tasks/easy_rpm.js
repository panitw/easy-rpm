...
function writeSpecFile(grunt, files, attrBasket, options) {
    ...
    for (i = 0; i < options.defattrScript.length; i++) {
        ...
        b.push("%defattr("
            + (defattr.fileMode ? defattr.fileMode : '-') + ', '
            + (defattr.user ? defattr.user : '-') + ', '
            + (defattr.group ? defattr.group : '-') + ', '
            + (defattr.dirMode ? defattr.dirMode : '-') + ')');
    }
...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask("easy_rpm", "Easily create RPM package to install files/directories", function () {
        ...
        var specFilepath = writeSpecFile(grunt, fileBasket, attrBasket, options);
